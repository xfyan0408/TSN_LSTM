...
class WindowDataset(Dataset):
    ...
    def __getitem__(self, i):
        x = self.arr[i:i+self.T]                 # (T,3)
        y = self.arr[i+self.T:i+self.T+self.H]   # (H,3)
...
class Forecaster(nn.Module):
    ...
    def __init__(self, H, d_model=32, tcn_layers=6, k=3, lstm_hidden=32, head_hidden=64, dropout=0.1):
        super().__init__()
        self.H = H
        self.mix = nn.Conv1d(3, d_model, kernel_size=1)  # ChannelMix(1×1)
    ...
    def forward(self, x):
        ...
        x = self.mix_act(self.mix(x))         # (B,d,T)
        x = self.tcn(x)                       # (B,d,T)
        ...
        cpu = self.h_cpu(h)
        mem = self.h_mem(h)
        y = torch.stack([bw, cpu, mem], dim=-1)  # (B,H,3)
        return y
